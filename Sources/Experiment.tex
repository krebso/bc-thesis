...
For the implementation of the benchmark into the \texttt{HistoPipe}\footnote{\url{https://gitlab.ics.muni.cz/rationai/digital-pathology/pipeline/histopipe}} pipeline, see \myref{Section}{sec:imple}.
...
We used the pooling layer based on the results of the experiment in \myref{Section}{sec:conv-vs-pool}.
...
For an explainability method to actively assist a pathologist, the explanations must be computed fast enough not to disrupt his workflow.
...
Still, given EU regulations from \myref{Section}{sec:need-for-xai}, we posit that we should evaluate them accordingly.
...
\myref{Table}{tab:gpu-util} shows the utilisation of a single A40 GPU card while the explanations are being computed.
...
\myref{Figure}{fig:occ-weighting-game-boxplot} shows the distribution of Occlusion Weighting Game ratios across different thresholds for candidate saliency maps.
