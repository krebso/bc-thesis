...
We used the pooling layer based on the results from \myref{Appendix}{sec:conv-vs-pool}.
...
We are unaware of any previous work focusing on GPU utilization of explainability methods. Still, given EU regulations on AI from \myref{Section}{sec:need-for-xai}, we posit that we should evaluate it accordingly.
...
Since this method relies on sensitivity, we will only use the $24,858$ positive tiles from the test split of the dataset from \myref{Section}{sec:dataset}.
To see the curve of the drop in the model's confidence, refer to \myref{Figure}{fig:road-curve}.
We present a boxplot portraying the distribution of our results in \myref{Figure}{fig:road-boxplot}.
...
\myref{Figure}{fig:weighting-game-boxplot} depicts the ratio distribution per positive tiles in the test part of our dataset.
...
Then, we use this binary mask map as annotation $A$ in \myref{Equation}{eq:wg}.
...
The agreement aligns with our a priori expectation from \myref{Subsection}{sub:hirescam} that Occlusion will attribute mainly regions corresponding to the highest activation maps values since only those are considered when computing the final score because of GMP.
...
Moreover, GradCAM++ reported very different results in \myref{Appendix}{sec:conv-vs-pool}, decreasing our trust in its internal mechanism.
...
Both methods detecting the same pro-cancer patterns aligns with their theoretical background presented in \myref{Section}{sec:xai-cnn}.
\myref{Figure}{fig:both-pos} and \myref{Figure}{fig:both-nuclear-density} show positive patterns detected by CAM and HiResCAM.
We present negative xPatterns identified by CAM in \myref{Figure}{fig:cam-neg}.
...
This observation is illustrated in \myref{Figure}{fig:both-pos}, and we posit it positively advocates for the model's trustworthiness. 
...
However, given other contextual information, it is disregarded, and the model rightly classifies the tile as non-cancerous, as depicted in \myref{Figure}{fig:ruda-annot}.

\begin{figure}
    \begin{center}
    \begin{minipage}{1\textwidth}
    \fcolorbox{RoyalBlue}{white}
      {\includegraphics[width=0.49\textwidth]{img/cam-pos.png}
      \includegraphics[width=0.49\textwidth]{img/hirescam-pos.png}}
    \end{minipage}
